...
#MainMenu,footer,header[data-testid="stHeader"],div[data-testid="stToolbar"],div[data-testid="stDecoration"]{display:none!important;visibility:hidden!important}
        ...
        for _,row in ev_rows.iterrows():
            ...
            st.markdown(f"""<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid #f1f5f9">
                ...
                <div style="text-align:right">
                    <div style="font-size:12px;font-weight:700">{fmt_s(row['revenue'])}</div>
                    <div style="font-size:10px;color:{vc};font-weight:600">{'↑' if va>0 else '↓'}{abs(va):.1f}% vs avg</div>
                </div>
            </div>""", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
