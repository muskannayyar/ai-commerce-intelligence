...
section[data-testid="stSidebar"] label { color: #6b7280 !important; font-size:12px !important; }
...
.stRadio label { font-size: 12px !important; color: #374151 !important; }
.stButton > button {
...
.kpi-card {
    ...
    padding: 18px 20px; margin-bottom: 10px; box-shadow: 0 1px 4px rgba(0,0,0,.05);
}
...
.chart-card {
    background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px;
    padding: 18px 20px; box-shadow: 0 1px 4px rgba(0,0,0,.05); margin-bottom: 14px;
}
...
with st.sidebar:
    ...
    view = st.selectbox("View", ["📊 Overview","📅 MoM Analysis","📆 Weekly Analysis","📣 Campaigns","📍 Geography"],
...
if "Overview" in view:
    ...
    with col_r:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        st.markdown('<div class="chart-title">Category Mix</div><div class="chart-sub">Revenue share — S$353.9K total</div>', unsafe_allow_html=True)
...
st.caption("Ask anything about your Shopee data · Powered by Claude")
